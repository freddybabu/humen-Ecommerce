...
from .forms import RegistrationForm, UserForm, VerifyForm, AddressBookForm
...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request, "Registered Successfully! Verify OTP to continue")
            return redirect('ver')
    context = {
        'form': form
    }
...
def user_otp_sign_in_validation(request, phone_number):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            messages.error(
                request, 'Multiple accounts with the same phone number')
            return redirect('home')
...
def add_to_wishlist(request, product_id):
    ...
    created = Wishlist.objects.get_or_create(
        user=request.user, product=myproduct)
    if created:
